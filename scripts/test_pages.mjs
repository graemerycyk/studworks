...
test('improvement policy distinguishes future default-on limited data from current processing', async () => {
  ...
  assert.match(privacy, /preserve the admitted response rather than replace it with a summary/);
  assert.match(privacy, /hub model and firmware/);
  assert.match(privacy, /port positions and their described roles/);
  assert.match(privacy, /hub names, serial numbers and Bluetooth identifiers/);
  assert.match(privacy, /Unknown building terminology is not itself a reason to discard/);
  assert.match(privacy, /bounded to the current building task/);
  assert.match(terms, /relevant current-build context/);
